...
## todo - change block_size to max_length
##        change data to txt  -  why? why not?

##
##  add/use CharTokenizer!!!!!
##
...
class CharDataset(Dataset):
    ...
    def encode_tokens( self, txt ):
        return [self.stoi[s] for s in txt]

    def decode_tokens( self, toks ):
        chars = [self.itos[i] for i in toks]
        print(chars)
        return "".join( chars )
...
def create_dataloader_v1(txt,
    ...
    # Create dataset
    dataset = CharDataset(txt, max_length)
...
def main():
    ...
    encoded = train_dataset.encode_tokens( start_context )
    print( encoded )
#=> [27, 1, 19, 53, 42, 6, 1, 27, 1, 19, 53, 42, 2]
    encoded_tensor = torch.tensor(encoded).unsqueeze(0)
    print( encoded_tensor )
#=> tensor([[27,  1, 19, 53, 42,  6,  1, 27,  1, 19, 53, 42,  2]])
    ...
    decoded_text = train_dataset.decode_tokens( out.squeeze(0).tolist() )
